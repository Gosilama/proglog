...
import (
	...
	"github.com/gosilama/proglog/internal/config"
	"github.com/gosilama/proglog/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (client api.LogClient, cfg *Config, teardown func()) {
	...
	clientTLSConfig, err := config.SetupTLSConfig(config.TLSConfig{CertFile: config.ClientCertFile, KeyFile: config.ClientKeyFile})
	require.NoError(t, err)
